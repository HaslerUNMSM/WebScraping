...
class DBWebscraping:
    ...
    def insert_webscraping(self, connection, carga):
        try:
            ...
            params = (carga["busqueda"], carga["busqueda_area"], carga["pagina"],
                      carga["url_principal"], carga["url_busqueda"], carga["id_keyword"])
...
class DBOferta:
    ...
    def insert_oferta(self, connection, oferta):
        try:
            mydb = connection.connect()
            cur = mydb.cursor()
            sql = "insert into Oferta (id_webscraping, titulo,empresa,lugar,salario,oferta_detalle,url_oferta,url_pagina,fecha_creacion,fecha_modificacion,id_anuncioempleo) values (%s,%s,%s,%s,%s,%s,%s,%s,current_date,current_date,%s)"
            params = (oferta["id_carga"], oferta["puesto"].strip(), oferta["empresa"].strip(), oferta["lugar"].strip(
            ), oferta["salario"].strip(), oferta["detalle"].strip(), oferta["url"], oferta["url_pagina"], oferta["id_anuncioempleo"])
            cur.execute(sql, params)
            mydb.commit()
...
class DBOfertadetalle:
    ...
    def update_ofertadetalle(self, connection, requisito):
        ...
        params = (requisito["descripcion_normalizada"],
                  requisito["iddescripcion"])
