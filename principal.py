...
import webscraping_convocatoriastrabajo
...
def url_busqueda_computrabajo(filtro):
    busqueda = "trabajo-de-" + \
        filtro.replace(" ", "-")+"-en-lima?q="+filtro.replace(" ", "%20")
    return busqueda


def url_busqueda_convocatoriastrabajo(filtro):
    busqueda = "buscar-empleo.php?q=" + \
        filtro.replace(" ", "+")+"&dep=0"
    return busqueda
...
def set_url_busqueda_convocatoriasTrabajo(carga, sitio, filtro):
    # MODIFICADO URL CONVOCATORIAS-TRABAJO
    carga["url_principal"] = sitio["WS_PORTAL_LABORAL_URL"]
    urlbusqueda = url_busqueda_convocatoriastrabajo(filtro)
    paginado = ""
    carga["url_prefix"] = carga["url_principal"] + urlbusqueda + paginado
    carga["url_sufix"] = ""
    carga["url_busqueda"] = carga["url_principal"] + urlbusqueda
...
def delati_convocatoriasTrabajo():
    controller = Controller()
    con = connect_bd()
    palabras = controller.getwords(con)
    for filtro in palabras:
        carga = {}
        carga["pagina"] = CONVOCATORIATRABAJO["WS_PORTAL_LABORAL"]
        carga["cant_paginas"] = CONVOCATORIATRABAJO["WS_PAGINAS"]
        carga["pagina_inicial"] = CONVOCATORIATRABAJO["WS_PAGINA_INICIAL"]
        carga["cant_ofertas"] = CONVOCATORIATRABAJO["WS_OFERTAS"]
        carga["busqueda_area"] = CONVOCATORIATRABAJO["WS_AREA"]
        carga["busqueda"] = ""
        carga["id_keyword"] = filtro[0]
        set_url_busqueda_convocatoriasTrabajo(
            carga, CONVOCATORIATRABAJO, filtro[1])
        carga["id_carga"] = controller.registrar_webscraping(con, carga)
        listaOferta = webscraping_convocatoriastrabajo.scraping_ofertas(con, carga["url_principal"], carga["url_prefix"], carga["url_sufix"],
                                                                        carga["pagina_inicial"], carga["cant_paginas"], carga["cant_ofertas"],
                                                                        carga["id_carga"])
        print(listaOferta)
